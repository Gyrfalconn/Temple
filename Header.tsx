              ...
              <div className="h-10 w-10 sm:h-12 sm:w-12 md:h-16 md:w-16 rounded-full bg-white flex items-center justify-center p-1 shadow-md border-2 border-amber-400 overflow-hidden">
                <img 
                  ...
                  onError={(e) => {
                    ...
                    if (parent) {
                      parent.innerHTML = '<span class="text-red-900 font-black text-lg md:text-xl">ॐ</span>';
                    }
                  }}
                />
              </div>
              ...
              <div className="h-10 w-10 sm:h-12 sm:w-12 md:h-16 md:w-16 rounded-full bg-white flex items-center justify-center p-1 shadow-md border-2 border-amber-400 overflow-hidden">
                <img 
                  ...
                  onError={(e) => {
                    ...
                    const parent = e.currentTarget.parentElement;
                    if (parent) {
                      parent.innerHTML = '<span class="text-red-900 font-black text-lg md:text-xl">🚩</span>';
                    }
                  }}
                />
              </div>
      ...
      <div className="bg-[#1a0f00] py-1.5 px-4 no-print border-t border-white/5">
        <div className="max-w-[1600px] mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 md:gap-4">
            <button
              onClick={() => setView('add')}
              className={`px-4 md:px-7 py-1.5 rounded-lg font-black text-[9px] md:text-[10px] uppercase tracking-widest transition-all bg-red-600 text-white hover:bg-red-700 shadow-md active:scale-95`}
            >
